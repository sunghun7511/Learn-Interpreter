...
Token check_next_token(const Token& token, const int kind2);
void set_token_pointer(char *p);
std::string DEBUG_kind_to_string(const int kind);
std::string kind_to_string(const int kind);
std::string kind_to_string(const CodeSet& cd);
int  get_line_number();
...
void statement();
void block();
double get_expression(const int kind1 = 0, const int kind2 = 0);
void expression(const int kind1, const int kind2);
void expression();
...
CodeSet check_next_code(const CodeSet& cd, const int kind2);
